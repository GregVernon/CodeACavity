...
% method = {'Direct', 'Decomposition', 'CG', 'Jacobi', 'AlternatingAndersonRichardson'};
method = {'Decomposition', 'CG', 'Jacobi', 'Mapped Jacobi', 'AlternatingAndersonRichardson'};

f = cell(length(N),length(method));
t = zeros(length(N),length(method));
for ii = 1:length(N)
    for m = 1:length(method)
        f{ii,m} = @() lidCavity(N(ii),1e2,method{m},1e-5,nSteps,false);
